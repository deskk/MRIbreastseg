...
def pred_and_save_masks_3d_divided(
    ...
    for i, batch in tqdm(enumerate(loader), total=len(loader)):
        ...
        if dataset.subject_id_list[list_index] != current_subject:
            volume_pred_array = volume_pred_sum / volume_pred_count
            ...
            np.save(
                ...
                volume_pred_array.astype(np.float32)
            )

            del volume_pred_sum
            del volume_pred_count
            gc.collect()
        ...
        # Initialize the arrays if None
        if volume_pred_sum is None:
            x_length, y_length, z_length = dataset.image_array_list[list_index].shape
            if n_classes == 1:
                volume_pred_sum = np.zeros((x_length, y_length, z_length), dtype=np.float32)
                volume_pred_count = np.zeros((x_length, y_length, z_length), dtype=np.float32)
            else:
                volume_pred_sum = np.zeros((n_classes, x_length, y_length, z_length), dtype=np.float32)
                volume_pred_count = np.zeros((n_classes, x_length, y_length, z_length), dtype=np.float32)
