...
ln -s external-duke-fgt/config_duke.json config.json
...
# --- Phase 0 ---
if [ -f external-duke-fgt/.phase0.done ]; then
    ...
    python external-duke-fgt/prepare_duke_nifti.py
    touch external-duke-fgt/.phase0.done
fi

# --- Phase 1 ---
if [ -f external-duke-fgt/.phase1.done ]; then
    ...
    touch external-duke-fgt/.phase1.done
fi

# --- Phase 2 ---
if [ -f external-duke-fgt/.phase2.done ]; then
    ...
    # python pipeline/phase2_mama-mia/scripts/postprocess_filter_noise.py # Disabled: no longer needed after fixing preprocessing
    touch external-duke-fgt/.phase2.done
fi

# --- Phase 3 ---
if [ -f external-duke-fgt/.phase3.done ]; then
    ...
    rm -rf inference_data/batch/images/* inference_data/batch/preds_breast/* inference_data/batch/preds_dv/*
    ...
    cd ../../../
    touch external-duke-fgt/.phase3.done
fi

# --- Phase 4 ---
if [ -f external-duke-fgt/.phase4.done ]; then
    ...
    touch external-duke-fgt/.phase4.done
fi

# --- Phase 5 ---
if [ -f external-duke-fgt/.phase5.done ]; then
    ...
    touch external-duke-fgt/.phase5.done
fi
...
echo "Generating random visualizations..."
conda activate breastseg
python external-duke-fgt/visualize_duke_pipeline.py
